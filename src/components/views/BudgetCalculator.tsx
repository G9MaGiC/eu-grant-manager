      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              <h1 className="text-2xl font-semibold text-[#F3F6FF]">Budget Calculator</h1>
              <p className="text-[#A9B3D0]">Plan and calculate your grant budget</p>
            </div>
          </div>
        </div>
      ...
      <div className="grid grid-cols-12 gap-6">
          ...
          <div className="card-dark p-6">
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full bg-[#161F32] border border-[#273155] rounded-xl px-4 py-3 text-[#F3F6FF] text-lg font-medium focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/50"
              />
            </div>

            {/* Budget Items */}
            <div className="space-y-3 mb-6">
              <div className="grid grid-cols-12 gap-3 text-[#A9B3D0] text-sm font-medium px-4">
                ...
                <div key={item.id} className="budget-row grid grid-cols-12 gap-3 items-center">
                  <div className="col-span-3">
                    <select
                      ...
                      className="w-full bg-[#161F32] border border-[#273155] rounded-lg px-3 py-2 text-[#F3F6FF] text-sm focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/50"
                    >
                  ...
                  <div className="col-span-3">
                    <div className="relative">
                      <Euro className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#A9B3D0]" />
                      <input
                        type="number"
                        value={item.amount || ''}
                        onChange={(e) => updateItem(item.id, 'amount', parseFloat(e.target.value) || 0)}
                        className="w-full bg-[#161F32] border border-[#273155] rounded-lg pl-9 pr-3 py-2 text-[#F3F6FF] text-sm focus:outline-none focus:ring-2 focus:ring-[#4F46E5]/50"
                      />
                    </div>
                  </div>
            ...
            <button 
              onClick={addItem}
              className="w-full py-3 border-2 border-dashed border-[#273155] rounded-xl text-[#A9B3D0] hover:text-[#F3F6FF] hover:border-[#4F46E5]/50 transition-colors flex items-center justify-center gap-2"
            >
            ...
            {/* Co-financing */}
            <div className="mt-6 pt-6 border-t border-[#273155]">
              <div className="flex items-center justify-between mb-4">
                <label className="text-[#A9B3D0] text-sm flex items-center gap-2">
                  <Percent className="w-4 h-4" />
                  EU Co-financing Rate
                </label>
                <span className="text-[#F3F6FF] font-medium">{coFinancingRate}%</span>
              </div>
              <input
                type="range"
                min="50"
                ...
                className="w-full h-2 bg-[#161F32] rounded-lg appearance-none cursor-pointer accent-[#4F46E5]"
              />
              <div className="flex justify-between text-[#A9B3D0] text-xs mt-2">
                <span>50%</span>
                <span>75%</span>
                <span>100%</span>
              </div>
        ...
        <div className="col-span-4 space-y-5">
          ...
          <div className="card-dark p-5">
            <h3 className="text-[#F3F6FF] font-semibold mb-4 flex items-center gap-2">
            ...
            <div className="space-y-4">
              <div className="p-4 bg-[#161F32] rounded-xl">
                <p className="text-[#A9B3D0] text-sm mb-1">Total Budget</p>
                <p className="text-[#F3F6FF] text-2xl font-bold">
                  {new Intl.NumberFormat('en-EU', { style: 'currency', currency: 'EUR' }).format(totalBudget)}
                </p>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div className="p-4 bg-[#4F46E5]/10 rounded-xl border border-[#4F46E5]/30">
                  <p className="text-[#4F46E5] text-sm mb-1">EU Funding</p>
                  <p className="text-[#F3F6FF] text-lg font-semibold">
                    {new Intl.NumberFormat('en-EU', { style: 'currency', currency: 'EUR', notation: 'compact' }).format(euContribution)}
                  </p>
                  <p className="text-[#4F46E5] text-xs">{coFinancingRate}%</p>
                </div>
              ...
              {categoryBreakdown.map((cat, i) => (
                <div key={i}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className="text-[#A9B3D0]">{cat.category}</span>
                    <span className="text-[#F3F6FF]">{cat.percentage.toFixed(1)}%</span>
                  </div>
                  ...
                  <p className="text-[#A9B3D0] text-xs mt-1">
          ...
          {/* Validation */}
          <div className="card-dark p-5">
            <h3 className="text-[#F3F6FF] font-semibold mb-4">Budget Validation</h3>
